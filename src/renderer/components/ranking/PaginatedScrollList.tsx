    ...
    <div className="w-full h-full relative">
      ...
      >
        {currentPageData.map((entry, index) => {
          ...
          if (rank) {
            if (rank === 1) {
              rankStyle = "text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-amber-500";
              rankBgStyle = "bg-black/70 border-2 border-yellow-400/50";
            } else if (rank === 2) {
              rankStyle = "text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-slate-200 to-gray-400";
              rankBgStyle = "bg-black/70 border-2 border-gray-400/50";
            } else if (rank === 3) {
              rankStyle = "text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-orange-300 to-amber-600";
              rankBgStyle = "bg-black/70 border-2 border-orange-400/50";
            } else {
          ...
          return (
            <div key={`${entry.score}-${currentPage}-${index}`} className="w-full h-full flex flex-col items-center justify-center">
              ...
              <div className="flex-shrink-0 h-6 flex items-center justify-center">
                {rank && (
                  <div className={`px-3 py-1 rounded-full ${rankBgStyle}`}>
                    <span className={rankStyle}>{rank}位</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 w-full min-h-0">
                <div className="w-full h-full bg-gradient-to-b from-slate-700 to-slate-800 border border-cyan-400/20 rounded-lg shadow-lg overflow-hidden relative">
                  {imageUrl ? (
                    <img 
                      src={imageUrl} 
